...
class Dino:   # 공룡 클래스 
    ...
    def update(self, userInput) -> None:
        ...
        if userInput[pygame.K_UP] and self.dino_jump :   # 이 때 점프 
            self.dino_run = False
    ...
    def run (self):
        self.image = self.run_img[self.step_index // 5]   # 10 0 과 1이 왔다갔다 
    ...
    def duck (self):
       self.image = self.duck_img[self.step_index // 5]   # 10 0 과 1이 왔다갔다 
       self.dino_rect = self.image.get_rect()
       self.dino_rect.x = self.X_POS
    ...
    def jump (self):
        ...
        if self.dino_jump:
            ...
            self.jump_vel -= 0.8
        if self.jump_vel < -self.JUMP_VEL :      # -9.0이 되버리면 점프 중단
            self.dino_jump = False
