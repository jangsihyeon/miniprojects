...
class qtApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowIcon(QIcon('./studyPyQt/addrbook.png'))
        ...
        self.btnSave.clicked.connect(self.btnSaveClicjed)
        self.tblAddress.doubleClicked.connect(self.tblAddressDoublClicked)
    ...
    def tblAddressDoublClicked(self):  #
    ...
    def btnSaveClicjed(self):   # 저장
        ...
        if fullName == '' or phoneNum =='':
            QMessageBox.warning(self, '주의', '이름과 핸드폰번호를 입력해주세요!')
            return # 진행불가
        else:
            self.conn = pymysql.connect(host='localhost', user='root', password='12345',
                                        db='miniproject', charset='utf8')
            if self.curIdx == 0:   # 신규 
            # 4개의 변수값 받아서 INSERT 쿼리문 만들기 
    ...
    def makeTable(self, rows):
        self.tblAddress.setColumnCount(5)        # 0.열개수  
        self.tblAddress.setRowCount(len(rows))   # 0.행개수 
        ...
        self.tblAddress.setColumnWidth(1, 70)     # 이름 열70
        ...
        for i , row in enumerate(rows):
            ...
            self.tblAddress.setItem(i, 2, QTableWidgetItem(phonNum))
