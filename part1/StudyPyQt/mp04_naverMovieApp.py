...
class qtApp(QWidget):
    ...
    def makeTable(self, items)-> None:
        ...
        self.tblResult.setEditTriggers(QAbstractItemView.NoEditTriggers)   # 컬럼 수정 금지 
        ...
        for i, post, in enumerate(items):                          # 0, 뉴스 ...
            ...
            director = post['director'].replace('|', ',')[:-1]     # 파이썬에서만 가능
            ...
            if img_url != '':                    # 빈값이면 포스터가 없음
                data = urlopen(img_url).read()   # 2진데이터 - 네이버 영화에 있는 이미지 다운 , 데이터 
                image = QImage()                 # 이미지 객체 
                image.loadFromData(data)

                # QtableWidget 이미지를 그냥 넣을 수 없음 QLabel()을 집어넣은 뒤 QLabel -> QTableWidget 
                imgLabel = QLabel()
                imgLabel.setPixmap(QPixmap(image))               # PyQt의 이미지 함수? -> 더 좋대 
